...
from scapy.fields import StrLenField, LEIntField, ByteField, ConditionalField, PacketField, StrNullField, StrNullFieldUtf16, MultipleTypeField, SignedByteField, LESignedShortField, LEShortField, LESignedIntField, LESignedLongField, LELongField
import six
            ...
            if cimType & CIM_ARRAY_FLAG:
                ...
                if cimType == CIM_TYPE_ENUM.CIM_ARRAY_STRING.value:
                    ...
                    for item in range(numItems):
                        ...
                        array.append(struct.unpack(unpackStrArray, heapData[:dataSizeArray])[0])
...
class DERIVATION_LIST(Packet):
    ...
    fields_desc = [
        ...
        StrLenField('ClassNameEncoding', b"", length_from=lambda pkt: pkt.EncodingLength - 4),
    ]
...
class QUALIFIER(Packet):
    ...
    fields_desc= [
        ...
        MultipleTypeField([
                (factory("QualifierValue"), lambda pkt, t=t: pkt.QualifierType & (~CIM_ARRAY_FLAG) == t)
...
class PROPERTY_LOOKUP_TABLE(Packet):
    ...
    fields_desc = [
        ...
        StrLenField('PropertyLookup', None, length_from=lambda pkt: pkt.PropertyCount * 8),
    ]
        ...
        for property in range(self.PropertyCount):
            ...
            while len(qualifiersBuf) > 0:
                ...
                if record.QualifierName & 0x80000000:
                    ...
                    qualifierName = ENCODED_STRING(heap[record.QualifierName:]).str_value()
...
class CLASS_HEAP(Packet):
    ...
    fields_desc = [
        ...
        StrLenField("HeapItem", b"", length_from=lambda pkt: pkt.HeapLength&0x7fffffff)
    ]
    def extract_padding(self, s):
        return b"", s
...
class CLASS_PART(Packet):
    ...
    fields_desc = [
        ...
        StrLenField("NdTable_ValueTable", b"", lambda pkt: pkt.ClassHeader.NdTableValueTableLength),
...
class INSTANCE_PROP_QUALIFIER_SET(Packet):
    ...
    fields_desc = [
        ByteField("InstPropQualSetFlag", None)
    ]
...
class INSTANCE_TYPE(Packet):
    ...
    fields_desc = [
        ...
        StrLenField('NdTable_ValueTable', b"", length_from=lambda pkt: pkt.CurrentClass.ClassPart.ClassHeader.NdTableValueTableLength),
...
class OBJECT_BLOCK(Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(# This block MUST be present if the ObjectFlags (section 2.2.6) octet has 0x04 bit flag set; otherwise, it MUST be omitted.
            PacketField("Decoration", None, DECORATION),
            lambda p: p.ObjectFlags & 0x04
        ),
    ...
    def parseClass(self, pClass: CLASS_AND_METHODS_PART, cInstance: INSTANCE_TYPE = None):
    ...
    def printClass(self, pClass : CLASS_AND_METHODS_PART, cInstance: INSTANCE_TYPE = None):
